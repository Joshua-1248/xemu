...
{
    /* Optional texture-pack procedural targets own their sampler override. */
    if (xemu_texture_packs_gl_apply_sampler_override(binding)) {
        return;
    }
...
{
    xemu_texture_packs_renderer_sync(pgraph_gl_texture_cache_flush);
    ...
    /* Optional texture-pack dynamics are serviced outside renderer state. */
    xemu_texture_packs_gl_refresh_dynamic();
    ...
    for (i=0; i<NV2A_MAX_TEXTURES; i++) {
        ...
        if (!pg->texture_dirty[i] && tbind) {
            ...
            if (reusable) {
                ...
                xemu_texture_packs_gl_refresh_binding(r->texture_binding[i]);
        ...
        if (key_out->binding == NULL) {
            ...
            xemu_texture_packs_gl_refresh_binding(key_out->binding);
        }
                              ...
                              uint64_t data_hash)
{
    ...
    case GL_TEXTURE_2D:
        if (f.linear) {
            ...
            const uint8_t *dump_data = converted ? converted : texture_data;
            unsigned int dump_stride = adjusted_pitch;
            if (converted && adjusted_width && adjusted_height) {
                size_t pixels = (size_t)adjusted_width * adjusted_height;
                if (pixels && converted_size / pixels == 4) {
                    dump_stride = adjusted_width * 4;
                }
            }
            xemu_texture_packs_gl_maybe_dump_guest32(
                data_hash, gl_target, 0, adjusted_width, adjusted_height,
                dump_stride, s.color_format, dump_data);
    ...
    case GL_TEXTURE_CUBE_MAP_NEGATIVE_Z: {
        ...
        for (level = 0; level < s.levels; level++) {
            ...
            if (f.gl_format == 0) { /* compressed */
                ...
                xemu_texture_packs_gl_maybe_dump_guest32(
                    data_hash, gl_target, level, tex_width, tex_height,
                    width * 4, s.color_format, converted);
                g_free(converted);
                if (s.cubemap && adjusted_width != s.width) {
                ...
                unsigned int pixel_bpp = f.bytes_per_pixel;
                unsigned int pixel_stride = pitch;
                if (converted && width && height) {
                    size_t pixels = (size_t)width * height;
                    if (pixels) {
                        pixel_bpp = converted_size / pixels;
                        pixel_stride = width * pixel_bpp;
                    }
                }
                unsigned int tex_width = width;
                unsigned int tex_height = height;
                ...
                if (s.cubemap && adjusted_width != s.width) {
                    ...
                    tex_width = s.width;
                    tex_height = s.height;
                    pixel_data += 4 * pixel_bpp + 4 * pixel_stride;
                }
                ...
                if (pixel_bpp == 4) {
                    xemu_texture_packs_gl_maybe_dump_guest32(
                        data_hash, gl_target, level, tex_width, tex_height,
                        pixel_stride, s.color_format, pixel_data);
                }
                                        ...
                                        uint64_t data_hash)
{
    ...
    bool replaced = xemu_texture_packs_gl_try_replace_bound_texture(
        data_hash, gl_target, s.levels, &is_animated);
    ...
    xemu_texture_packs_gl_binding_created(
        ret, data_hash, s.width, s.height, is_animated,
        is_animated && s.levels > 1);
    return ret;
}
...
static void texture_binding_destroy(gpointer data)
{
    ...
    binding->refcnt--;
    if (binding->refcnt == 0) {
        xemu_texture_packs_gl_binding_destroy(binding);
...
void pgraph_gl_init_textures(NV2AState *d)
{
    ...
    xemu_texture_packs_set_backend(XEMU_TEXTURE_PACKS_BACKEND_GL);
}
...
void pgraph_gl_texture_cache_flush(void)
{
     ...
     */
    if (g_nv2a == NULL ||
        xemu_texture_packs_get_backend() != XEMU_TEXTURE_PACKS_BACKEND_GL) {
        return;
    }
